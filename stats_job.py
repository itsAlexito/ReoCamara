...
async def stats_job(application, chat_id=None, clear_after=True):
    logger.warning(f"[stats_job] Llamado con chat_id={chat_id}, clear_after={clear_after}")
    """Tarea diaria que envía las estadísticas y limpia el log.
    Si chat_id se pasa, envía ahí; si no, intenta obtener TARGET_CHAT_ID desde:
      1) application.bot_data['TARGET_CHAT_ID']
      2) la variable de entorno TARGET_CHAT_ID
    clear_after: si True borra el log al final; si False lo mantiene (usado por /stats manual)."""

    if chat_id is None:
        # Prefer bot_data (allows runtime configuration); fallback to env var
        bot_data_target = None
        try:
            bot_data_target = application.bot_data.get("TARGET_CHAT_ID")
        except Exception:
            bot_data_target = None

        if bot_data_target:
            chat_id = int(bot_data_target)
        else:
            env_target = os.getenv("TARGET_CHAT_ID")
            chat_id = int(env_target) if env_target else None
    ...
    # --- Limpieza ---
    try:
        if clear_after and os.path.exists(LOG_FILE):
            os.remove(LOG_FILE)
        # image_path puede ser None si no se generó ninguna imagen
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Llamamos a stats_job sin depender de un objeto `config` externo
    await stats_job(app, chat_id=chat_id, clear_after=False)
